...
#' @param show_intercept (logical) drop intercept term from all models?
#' @param model_name (character) name of variable
#' @param point_args (list) extra arguments to pass to \code{\link{geom_point}}
#' @param segment_args (list) extra arguments to pass to \code{\link{geom_segment}}
...
#'
#' m2 <- update(m1, . ~ . - disp)
#' dwplot(list(full=m1,nodisp=m2),drop_intercept=TRUE)
#'
#' # scaled model coefficients
#'
#' m1s <- update(m1, . ~ scale(wt)+scale(cyl)+scale(disp))
#' dwplot(m1s)+geom_vline(xintercept=0,lty=2)
#'
...
#'
#' # modify geom and segment arguments
#' dwplot(m1, segment_args=list(lwd=2),
#'    point_args=list(size=6,pch=21,fill="white"))
#'
