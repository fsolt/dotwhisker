...
#' @param whisker_args When \code{style} is "dotwhisker", a list of arguments specifying the appearance of the whiskers representing the confidence intervals.  For supported arguments, see \code{\link[ggplot2]{geom_linerange}}.
...
#' @param line_args When \code{style} is "distribution", a list of arguments specifying the appearance of the line marking the confidence interval beneath the normal distribution.  For supported arguments, see \code{\link[ggplot2]{geom_linerange}}.
...
dwplot <- function(x,
                ...
                whisker_args = list(linewidth = .5),
                dist_args = list(alpha = .5),
                line_args = list(alpha = .75, linewidth = 1),
                ...) {
    # argument checks
    if (length(style) > 1)
        style <- style[[1]]
...
dw_tidy <- function(x, ci, by_2sd, margins, ...) {
    ...
    ## tidy a single model object into a broom-style data frame
    tidy_one <- function(model) {
        if (margins) {
            d <- avg_slopes(model, conf_level = ci)
            if (!is.null(d$group))
                d <- d[!duplicated(d$term), ] # marginal effects for the first category of y
        } else {
            d <- standardize_names(parameters(model, ci, conf.int = TRUE, ...), style = "broom")
        }
        if (by_2sd)
            d <- by_2sd(d, get_dat(model))
        d
    }

    if (is.data.frame(x)) {
        # x is a data frame: add confidence bounds if missing
        df <- x
        ...
        if (is.null(names(x)))
            names(x) <- paste0("Model ", seq_along(x))
        df <- list_rbind(purrr::map(x, tidy_one), names_to = "model")
    } else {
        # single model object
        df <- tidy_one(x)
    }
...
geom_dwdist <- function(data = NULL, df1, line_args, dist_args) {
    ...
    l2 <- layer(
        ...
        geom = "linerange",
        show.legend = FALSE,
        params = c(line_args, list(orientation = "y"))
    )
    return(list(l1, l2))
}
...
geom_dw <- function(df, point_args, segment_args, dodge_size) {
    ...
    # Merge any user-supplied (unnamed) aes into the base mapping and keep the
    # remaining named entries as layer parameters.
    split_args <- function(args, base_aes) {
        added <- args[names(args) == ""]
        if (length(added)) {
            mapping <- modifyList(base_aes, added[[1]])
    ...
    point <- split_args(point_args, aes(
        y = stats::reorder(term, y_ind),
        x = estimate,
        group = interaction(model, term),
        color = model
    ))
    segment <- split_args(segment_args, aes(
        y = stats::reorder(term, y_ind),
        xmin = conf.low,
        xmax = conf.high,
        group = interaction(model, term),
    ...
    l2 <- layer(
        ...
        geom = "linerange",
        show.legend = FALSE,
        params = c(segment$params, list(orientation = "y"))
    )
    return(list(l2, l1))
}
