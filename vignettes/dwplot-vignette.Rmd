...
Extracting estimates from most model objects is straightforward, since many functions generate results that can be extract by `coef()`. Further, the `broom` package [@Robinson2015] automates the process of extracting estimates for many common model objects. 
...
The input data frame can also consist of estimates of other quantities of interest, such as margins and predicted probabilities, rather than coefficients. 

```{r message= FALSE}
# Create a data.frame of marginal effects
library(mfx)
m3 <- logitmfx(formula = am ~ mpg + wt + disp + gear, data = mtcars) 
...
The `dotwhisker::dwplot` function is very easy to use. It takes just three arguments: `df`, `alpha`, and `dodge_size`.  `df` is the data frame of estimates to be plotted (as described above). `alpha` is the significance level of the confidence intervals to be spanned by the whiskers: its default value is .05, implying a 95% confidence interval. `dodge_size` is used to adjust the space between the estimates of one variable when  multiple models are presented in a single plot.
...
`dwplot` can also be used to create plots that present multiple models (e.g., across different samples) in little space while also facilitating cross-model comparisons. To do this, one first needs to append the data frame of estimates from the new model to that of the old model and create an additional column `model` that identifies the two models.


```{r fig.width= 7, fig.height=5, warning=FALSE, message=FALSE}
# Compare results across subsets of data
by_trans <- mtcars %>% group_by(am) %>%
    do(tidy(lm(mpg ~ wt + cyl + disp + gear, data = .))) %>% rename(model=am) %>%
...
m4 <- lm(mpg ~ wt + cyl + disp + vs, data = mtcars)         # same as m1
m5 <- lm(mpg ~ wt + cyl + disp + vs + hp, data = mtcars)    # add another predictor
m6 <- lm(mpg ~ wt + cyl + disp + vs + hp + am, data = mtcars) # and another 

# Tidy estimates, rescale, and omit intercepts
m4_df <- tidy(m4) %>% by_2sd(mtcars) %>% filter(term != "(Intercept)") 
m5_df <- tidy(m5) %>% by_2sd(mtcars) %>% filter(term != "(Intercept)") 
m6_df <- tidy(m6) %>% by_2sd(mtcars) %>% filter(term != "(Intercept)") 
...
An alternate use of dot-and-whisker plots is to compare the estimated coefficients for a single predictor across many models or datasets: Andrew Gelman calls such plots "[the secret weapon](http://andrewgelman.com/2005/03/07/the_secret_weap/)" analysis by Andrew Gelman.  They are easy to make with `dwplot`.
