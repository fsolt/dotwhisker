...
Extracting estimates from most model objects is straightforward, since many functions generate results that can be extract by `coef()`. Further, the `broom` package [@Robinson2014] automates the process of extracting estimates for many common model objects. 
...
At last, we show how to create a "small multiple" with `dotwhisker` functions. "Small multiple" is a type of plots presenting estimates of each variable with different conditions (estimated with different data or models) in a separate panel. Each panel has its own scale. @Kastellec2007[766] believe the small multiple maximize the efficiency of presentation and the convenience for cross-model comparison. To create a small multiple with `dwplot`, we first needs to splite the orginial estimate dataset into pieces in which there are estimates based on mutliple conditions but only for one variable. We offer a function `smul` to convenitnely split a `tidy` data.frame to a list of single variable datasets. Then, with the loop funciton, one can create a list of `dwplot`s for each dataset. Users can freely extract several or all the plots into one page to be the small multiple plot. The `multiplot` function by [\emph{Cookbook for R}](http://www.cookbook-r.com/Graphs/Multiple_graphs_on_one_page_(ggplot2)/) provide an easy way to combine the plots, which we also include in the `dotwhisker` package. 
